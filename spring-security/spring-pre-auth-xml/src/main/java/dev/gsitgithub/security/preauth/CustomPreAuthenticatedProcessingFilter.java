...
//			response.sendRedirect("/accessdenied");
	...
	protected Object getPreAuthenticatedCredentials(HttpServletRequest request) {
		String principalParam = request.getParameter(principalRequestHeader);
		String principalHeader = request.getHeader(principalRequestHeader);
		String principalSession = (String) request.getSession().getAttribute(principalRequestHeader);
		
		if (principalParam != null){
        	request.getSession().setAttribute(principalRequestHeader, "mysession");
        	principalSession = principalRequestHeader;
        }
		//if(principalSession == null)
        	//throw new AccessDeniedException("Access Denied in Preauth filter.");
